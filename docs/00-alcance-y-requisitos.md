...
| Id | Requisito |
|---|---|
| RF-01 | Barrido periódico de licitaciones activas (cada 2 horas) con selección por reglas de afinidad; con la ficha se suman las señales estructurales —canon mensual, tipo de proceso, ítems— y se decide sobre el total (D-41). |
...
| RF-03 | Clasificación automática: vertical (citas y contactabilidad, activos fijos, gestión documental, calidad y acreditación, control de asistencia, gestión de mantenimiento, droguería y bodega farmacéutica, desarrollo web y plataformas, otros), tipo de comprador (hospital, servicio de salud, DAS o salud municipal, municipio, universidad o CFT, servicio público, otro), señales de incumbente y de oportunidad detectadas en el texto (D-40). |
| RF-04 | Tablero: lista única con búsqueda por texto (código, nombre, organismo, descripción) y filtros por estado de revisión, vertical, tipo de comprador, tipo de proceso, región y tramo de monto (incluye «sin monto publicado»). Orden por cierre, afinidad, monto o publicación. Las que están bajo el umbral vigente se ocultan por defecto, con casilla para verlas (D-42, D-46); igual las cerradas en el portal o con cierre vencido, salvo que tengan oferta presentada (D-50). Los filtros por fecha de cierre, afinidad mínima y señales de incumbente del diseño original no se construyeron: el orden por cierre y las etiquetas en la fila los cubren. |
...
| RF-09 | Reglas de afinidad editables desde la interfaz (Administrador): palabras clave por vertical con peso, exclusiones, umbral de selección, rango de montos, tipos de proceso, banda de canon mensual y restas de la ficha (D-41). Con vista previa: "con estas reglas, hoy se seleccionarían N licitaciones". |
...
| RF-11 | Exportación del tablero filtrado a Excel (CSV con BOM y punto y coma, D-34). |
